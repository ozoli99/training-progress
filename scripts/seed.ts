...
async function main() {
  ...
  while (cursor <= today) {
    ...
    const isStrengthDay =
      day === 1 || day === 3 || day === 5 || (day === 2 && rnd() > 0.6);
      ...
      for (const lift of lifts) {
        ...
        const setRows = Array.from({ length: setCount }).map(() => {
          ...
          const weight = Math.max(
            20,
            Math.round((lift.top + jitter(5)) * 2) / 2
          );
          const rpe = Math.min(
            10,
            Math.max(6, Math.round((7.5 + jitter(1.2)) * 2) / 2)
          );
    ...
    if (isRunDay) {
      ...
      const weeksFromStart = Math.floor(
        (cursor.getTime() - start.getTime()) / (7 * 86400000)
      );
      const improvement = weeksFromStart * 8;
      const timeSec = Math.max(
        20 * 60,
        Math.round(base - improvement + jitter(45))
      );

      await db
        .insert(sets)
        .values([{ id: nanoid(), sessionLogId: runLogId, timeSec }]);
    }
  ...
  for (const off of recentDays) {
    ...
    const [{ c: hasLog }] = await db
      ...
      .from(sessionLogs)
      .where(sql`date = ${iso} and exercise_id = ${squatId}`);
      ...
      await db.insert(sets).values([
        {
          ...
          sessionLogId: logId,
          reps: 5,
          weight: Math.round((squatTop + jitter(3)) * 2) / 2,
        },
        {
          id: nanoid(),
          sessionLogId: logId,
          reps: 5,
          weight: Math.round((squatTop - 2 + jitter(3)) * 2) / 2,
        },
        {
          id: nanoid(),
          sessionLogId: logId,
          reps: 5,
          weight: Math.round((squatTop - 4 + jitter(3)) * 2) / 2,
        },
      ]);
    }
  }
  ...
  console.log(
    `Seeded: ${exCount} exercises, ${logCount} session logs, ${setCount} sets over ~60 days.`
  );
}
