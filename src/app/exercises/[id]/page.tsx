...
type Log = {
    ...
    date: string;
    exercise?: Exercise;
    sets: Array<{ weight?: number; reps?: number; timeSec?: number; rpe?: number }>;
};

const est1RM = (reps?: number, weight?: number) => (!reps || !weight ? 0 : weight * (1 + reps / 30));
const volumeOf = (log: Log) => log.sets.reduce((a, s) => a + (s.reps && s.weight ? s.reps * s.weight : 0), 0);
const best1RMOf = (log: Log) => Math.max(0, ...log.sets.map(s => est1RM(s.reps, s.weight)));
const totalTimeOf = (log: Log) => log.sets.reduce((a, s) => a + (s.timeSec ?? 0), 0);

function unitMeta(unit: Unit) {
    switch (unit) {
        case "weight_reps":
            return { label: "weight × reps", icon: <Dumbbell className="h-4 w-4" />, help: "Load and repetitions per set." };
        case "time":
            return { label: "time", icon: <Timer className="h-4 w-4" />, help: "Track total duration in seconds." };
        default:
            return { label: "reps", icon: <Hash className="h-4 w-4" />, help: "Count-only movements." };
    }
}
...
export default function ExerciseDetailPage() {
    ...
    const [metric, setMetric] = React.useState<"volume" | "one_rm">("one_rm");
    ...
    const series = React.useMemo(() => {
        ...
        return sorted.map(l => ({
            x: l.date,
            y: metric === "volume" ? volumeOf(l) : unit === "time" ? totalTimeOf(l) : best1RMOf(l),
        }));
    }, [logs, metric, unit]);
        ...
        <div className="grid gap-6">
            <div className="rounded-2xl border bg-gradient-to-b from-muted/60 to-background p-6 md:p-8 relative overflow-hidden">
                <div
                    ...
                    style={{
                        backgroundImage: "radial-gradient(1px 1px at 20px 20px, hsl(var(--muted-foreground)) 1px, transparent 1px)",
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-3">
                    ...
                    {unit === "time" ? (
                        <KPI label="Total Time" value={`${Math.round(totalTime / 60).toLocaleString()} min`} subtle={`${totalTime} sec`} />
                    ) : (
                        <>
                            <KPI label="Total Volume" value={Math.round(totalVolume).toLocaleString()} />
                            <KPI label="Best Est. 1RM" value={Math.round(best1RM).toLocaleString()} subtle={latest && best1RMOf(latest) === best1RM ? "PR in last session 🎉" : undefined} />
                        </>
                    )}
                </div>
            </div>
                                ...
                                .map((l) => {
                                    ...
                                    return (
                                        <tr key={l.id} className="border-t">
                                            ...
                                            {unit !== "time" ? (
                                                <td className="py-2 px-3">{Math.round(bestSet).toLocaleString()} kg (est. 1RM)</td>
                                            ) : (
                                                <td className="py-2 px-3">{Math.round(totalTime / 60)} min</td>
                                            )}
                                            {unit !== "time" && (
                                                <td className="py-2 px-3">{Math.round(vol).toLocaleString()}</td>
                                            )}
                                            <td className="py-2 px-3 hidden sm:table-cell text-muted-foreground">—</td>
                                        </tr>
                                    );
                            })}
                        </tbody>
...
function KPI({ label, value, subtle }: { label: string; value: React.ReactNode; subtle?: string }) {
    return (
