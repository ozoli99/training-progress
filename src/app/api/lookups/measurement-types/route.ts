// api/lookups/measurement-types/route.ts
...
export const GET = withApiAuth(
  async () => {
    const items = await lookupsService.listMeasurementTypes();
...
export const POST = withApiAuth(
  async (req: AuthedRequest) => {
    ...
    const row = await lookupsService.upsertMeasurementType(input);
...
export const DELETE = withApiAuth(
  async (req: AuthedRequest) => {
    ...
    await lookupsService.deleteMeasurementType(input);
    return json.ok({}, 204);
  },
  { scope: "org", minRole: "org:admin" }
);
