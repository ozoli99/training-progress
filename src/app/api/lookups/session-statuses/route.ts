// api/lookups/session-statuses/route.ts
...
export const GET = withApiAuth(
  async () => {
    const items = await lookupsService.listSessionStatuses();
...
export const POST = withApiAuth(
  async (req: AuthedRequest) => {
    ...
    const row = await lookupsService.upsertSessionStatus(input);
...
export const DELETE = withApiAuth(
  async (req: AuthedRequest) => {
    ...
    await lookupsService.deleteSessionStatus(input);
    return json.ok({}, 204);
  },
  { scope: "org", minRole: "org:admin" }
);
