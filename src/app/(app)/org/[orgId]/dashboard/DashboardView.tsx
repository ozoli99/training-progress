...
/** ---------------------------
 * Shapes that mirror service returns
 * (Keep loose enough to compile without importing server types)
 * -------------------------- */
type ServiceKpis = {
...
type TrendSeries = {
  labels: string[]; // e.g., weeks or dates
...
function QuickActions({
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row">
      <Button onClick={onPlan}>Plan session</Button>
      <Button variant="outline" onClick={onLog}>
        Log workout
      </Button>
      <Button variant="ghost" onClick={onAddAthlete}>
        Add athlete
      </Button>
    </div>
  );
...
export default function DashboardView({ orgId }: Props) {
  ...
  // Derived client-only values (from service shape)
          ...
          {loading || !k ? (
            ...
            <>
              <KPICard
                ...
                trend={0} // hook up prev-range delta later
                spark={trend}
              />
              <KPICard
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              <QuickActions
                onPlan={() => router.push(`/org/${orgId}/sessions`)}
                onLog={() => router.push(`/org/${orgId}/log`)}
                onAddAthlete={() => router.push(`/org/${orgId}/athletes`)}
              />
            </CardContent>
          </Card>
