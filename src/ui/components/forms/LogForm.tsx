    ...
    return (
        <div className="p-4 md:p-0">
            ...
            <Formik<LogCreate>
                enableReinitialize
                initialValues={
                    mounted && typeof window !== "undefined" && localStorage.getItem(DRAFT_KEY)
            ...
            >
                {({ values, errors, setFieldValue, setFieldTouched, isValid, touched, isSubmitting }) => {
                    ...
                    const totalVolume = sum(values.sets.map(s => s.reps && s.weight ? s.reps * s.weight : 0));
                    const best1RM = Math.max(0, ...values.sets.map(s => est1RM(Number(s.reps), Number(s.weight))));
        ...
        setFieldValue("exerciseId", v);
                            ...
                            <div className="flex flex-wrap items-center gap-3 text-sm">
                                <span className="text-muted-foreground">
                                    {unit === "weight_reps" ? "This exercise tracks weight × reps." : unit === "reps" ? "This exercise tracks reps only." : "This exercise tracks time (seconds)."}
                                </span>
                                <span className="hidden sm:inline text-muted-foreground">•</span>
                                <span className="text-muted-foreground">
                                    <strong>Summary:</strong>{" "}
                                    {unit === "time" ? `${sum(values.sets.map(s => Number(s.timeSec ?? 0)))} sec total` : `Volume ${Math.round(totalVolume)}${unit === "reps" ? " reps" : ""}${unit === "weight_reps" ? "" : ""}${unit === "weight_reps" ? ` · est. 1RM ${Math.round(best1RM)}` : ""}`}
                                </span>
                                {autosavedAt && (
                                    <>
                                        <span className="hidden sm:inline text-muted-foreground">•</span>
                                    ...
                                    <div className="grid gap-3">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex gap-2">
                                                <Button type="button" variant="secondary" size="sm" onClick={() => push({ reps: 5, weight: 50 })}>
...
function SetRow({ index, unit, onRemove }: { index: number; unit: Exercise["unit"]; onRemove: () => void }) {
    return (
        <div className="grid grid-cols-12 gap-2 items-end">
            {(unit === "weight_reps" || unit === "reps") && (
                <div className="col-span-3 sm:col-span-2">
                    <Label htmlFor={`sets.${index}.reps`}>Reps</Label>
                    <Field as={Input} id={`sets.${index}.reps`} name={`sets.${index}.reps`} type="number" min={1} />
            ...
            {unit === "weight_reps" && (
                <div className="col-span-3 sm:col-span-2">
                    ...
                    <Field as={Input} id={`sets.${index}.weight`} name={`sets.${index}.weight`} type="number" min={0} step="0.5" />
            ...
            {unit === "time" && (
                <div className="col-span-4 sm:col-span-3">
                    ...
                    <Field as={Input} id={`sets.${index}.timeSec`} name={`sets.${index}.timeSec`} type="number" min={0} />
            ...
            <div className="col-span-3 sm:col-span-2">
                ...
                <Field as={Input} id={`sets.${index}.rpe`} name={`sets.${index}.rpe`} type="number" min={1} max={10} step="0.5" />
            ...
            <div className="col-span-3 sm:col-span-1 flex justify-end">
                <Button type="button" variant="ghost" onClick={onRemove} aria-label="Remove set">
